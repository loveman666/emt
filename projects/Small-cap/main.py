# coding=utf-8
# pyright: reportWildcardImportFromLibrary=false, reportMissingImports=false, reportUndefinedVariable=false
from __future__ import print_function, absolute_import, unicode_literals
import os
from gm.api import *  # 掘金官方模板要求的通配导入
...
def _get_close_price(symbol, end_time, adjust_end_time):
    """获取标的最新收盘价；停牌/无数据时返回 None。"""
    bars = history_n(symbol=symbol, frequency='1d', count=1, end_time=end_time,
                     fields='close', adjust=ADJUST_PREV,
                     adjust_end_time=adjust_end_time, df=False)
    if not bars:
        return None
    return bars[0]['close']
    ...
    if context.now.month!=pd.Timestamp(last_date).month:
        ...
        for position in positions:
            ...
            if symbol not in to_buy:
                new_price = _get_close_price(symbol, now_str, context.backtest_end_time)
                if new_price is None:
                    print('无法获取 {} 的收盘价，跳过平仓。'.format(symbol))
                    continue
        ...
        for symbol in to_buy:
            ...
            new_price = _get_close_price(symbol, now_str, context.backtest_end_time)
            if new_price is None:
                print('无法获取 {} 的收盘价，跳过买入。'.format(symbol))
                continue
            # # 当前价（tick数据，免费版本有时间权限限制；实时模式，返回当前最新 tick 数据，回测模式，返回回测当前时间点的最近一分钟的收盘价）
            # new_price = current(symbols=symbol)[0]['price']
...
if __name__ == '__main__':
    ...
    '''
    # 从环境变量读取 token，避免硬编码泄露；本地调试可临时填回 fallback
    token = os.environ.get('GM_TOKEN', '')
    if not token:
        raise RuntimeError('请先设置环境变量 GM_TOKEN（掘金账号 token），避免硬编码泄露。')
    run(strategy_id='25d2c353-5f5b-11f1-b93a-2c7ba0a58a6c',
        filename='main.py',
        mode=MODE_BACKTEST,
        token=token,
